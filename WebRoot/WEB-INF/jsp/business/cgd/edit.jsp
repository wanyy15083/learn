...
function yycgdmxaddshow(){
	var sendUrl="${baseurl}/cgd/addcgdmx.action?yycgdCustom.id=${yycgdCustom.id}";
...
var columns = [ [{
	...
	formatter:function(value,row,index){
		if(row.id){
			return '<input type="text" name="yycgdmxList['+index+'].cgl" value="'+(value?value:"")+'" />';
		}else{
			return value;
		}
...
//加载datagrid（yycgdmxlist）
	 ...
	 $(function() {
		$('#yycgdmxlist').datagrid({
			...
			onBeforeLoad:function(index,field,value){//加载datagrid之前执行unselectAll方法，取消选中的行
...
<BODY>
<!-- 采购单主信息保存form -->
<form id="yycgdsaveForm" name="yycgdsaveForm" method="post">
...
<TABLE border=0 cellSpacing=0 cellPadding=0 width="100%" bgColor=#c4d8ed>
	...
	<!-- 采购单明细form -->
	<form id="yycgdmxForm" name="yycgdmxForm" method="post" action="${ baseurl }/cgd/savecgdmx.action">
	<!-- 选中的药品id，逗号分割 -->
