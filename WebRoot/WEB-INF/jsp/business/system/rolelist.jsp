   ...
   var authorize_role = function(id) {
	...
	var columns = [ [ /* {
		...
		formatter:function(value,row,index){
			...
			/* if (row.groupid == ${ groupList.dictinfo.id }) {
				return ${groupList.dictinfo.info};
			} */
		}
	} , {
		field : 'opt1',
		...
		width : 130,
		formatter:function(value,row,index){
			return "<a href='${baseurl}/sys/auth/roleauth.action?roleId="+row.id+"' target='_blank'>角色授权</a>";
	...
	function initGrid() {
		$('#sysrolelist').datagrid({
			...
			striped : true,
			//collapsible : true,
			url : '${baseurl}/sys/auth/rolelist_result.action',
			//sortName : 'code',
			//sortOrder : 'desc',
			//remoteSort : false,
			idField : 'id',
